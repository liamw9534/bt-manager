...
class BTDevice(BTGenericDevice):
    ...
    * **Address(str)**: The Bluetooth device address of the
        remote device.
    * **Name(str)**: The Bluetooth remote name. This value can not
        be changed. Use the Alias property instead.
    * **Icon(str)**: Proposed icon name according to the freedesktop.org
        icon naming specification.
    * **Class(str)**: The Bluetooth class of device of the remote device.
    * **UUIDs(list{str})**: List of 128-bit UUIDs that represents the
        available remote services.
    * **Paired(boolean)**: Indicates if the remote device is paired.
    * **Connected(boolean)**: Indicates if the remote device is connected.
    * **Trusted(boolean)**: Indicates if the remote device is trusted.
    * **Alias(str)**: The name alias for the remote device. The alias can
        be used to have a different friendly name for the remote device. In
        case no alias is set, it will return the remote device name. Setting
        an empty string as alias will convert it back to the remote device
        name. When reseting the alias with an empty string, the emitted
        :py:attr`.SIGNAL_PROPERTY_CHANGED` signal will show the remote
        name again.
    * **Nodes(list{str})**: List of device node object paths.
    * **Adapter(str)**: The object path of the adpater the device belongs to.
    * **LegacyPairing(boolean)**: Set to true if the device only supports the
        pre-2.1 pairing mechanism. This property is useful in the
    ...
    """
    :function signal(signal_name, user_arg): This signal will be sent when a
        low level disconnection to a remote device has been requested. The
        actual disconnection will happen 2 seconds later.
    """
    SIGNAL_NODE_CREATED = 'NodeCreated'
    """
    :function signal(signal_name, user_arg, node_path): Signal notifying
    ...
    def discover_services(self, pattern=''):
        ...
        Refer to :py:class:`BTDiscoveryInfo` to decode each XML service record.
